...
class MainWindow(FluentWindow):
    ...
    def _sync_ma_slot_combo(self, _slot_idx=None):
        ...
        Slots in _active_scraping_slots are shown with a warning suffix and are
        made non-selectable so the user cannot assign Main Account to a scraping phone."""
        mp  = self.main_account_page
        dp  = self.dashboard_page
        ...
        # Use the explicitly tracked set (managed by _launch_scraping,
        # _stop_single_phone, _stop_all, and _on_phone_finished) so that
        # the label clears immediately when stop is requested, not only after
        # the worker thread exits.
        scraping_slots = getattr(self, "_active_scraping_slots", set())

        n = dp.active_slot_count()
        for i in range(n):
            if i in scraping_slots:
                label = self._phone_label(i) + "  🔴 scraping"
            else:
                label = self._phone_label(i)
            mp.combo_ma_slot.addItem(label, userData=i)
            mp.combo_ma_slot.setItemEnabled(i, i not in scraping_slots)

        # Restore selection; fall back to 0 if the previously selected slot
        # was removed (visible slots shrank below it).
        # If the current selection is now disabled, move to first free slot.
        idx = mp.combo_ma_slot.findData(prev)
        if idx < 0:
            idx = 0
        if idx < n and idx in scraping_slots:
            idx = next((j for j in range(n) if j not in scraping_slots), idx)
    ...
    def _launch_scraping(self, cfg: dict, assigned: list, phone_targets: list, total_targets: int):
        ...
        if not self._workers:
            ...
            # Refresh MA slot combo so any slot now scraping becomes non-selectable
            self._sync_ma_slot_combo()
    ...
    def _start_main_account(self):
        ...
        # ── Guard: forbid starting MA on a slot that is actively scraping ────
        scraping_slots = {w.phone_index for w in self._workers if w.isRunning()}
        if slot in scraping_slots:
            label = self._phone_label(slot)
            InfoBar.error(
                "Slot Conflict",
                f"{label} is currently running a scraping session. "
                "Stop scraping on that slot first, or choose a different phone slot for Main Account.",
                isClosable=True,
                duration=8000,
                parent=self,
            )
    ...
    def _stop_all(self):
        ...
        # Clear tracking set immediately so MA slot combo unlocks right away
        self._active_scraping_slots = set()
        self._sync_ma_slot_combo()
    ...
    def _stop_single_phone(self, phone_idx: int):
        ...
        for w in self._workers:
            if w.phone_index == phone_idx:
                ...
                # Remove from tracking set immediately so MA slot combo unlocks
                self._active_scraping_slots.discard(phone_idx)
                self._sync_ma_slot_combo()
                break
    ...
    def _on_phone_finished(self, phone_idx: int, count: int):
        ...
        # Slot is no longer scraping — re-enable it in the MA slot combo
        self._active_scraping_slots.discard(phone_idx)
        self._sync_ma_slot_combo()
    ...
    def _reset_ui_after_done(self):
        ...
        for btn in self.dashboard_page.stop_phone_btns:
            btn.setEnabled(False)
        # Refresh MA slot combo: scraping slots are now free again
        self._sync_ma_slot_combo()
